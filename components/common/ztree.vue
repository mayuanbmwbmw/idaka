<template>
    ...
    <div>
        <div class="addParent" v-if="edit && !noGroup">
        ...
        <no-ssr>
            <Modal class="detail_modal" v-model="detail_modal" width="360" :mask="true">
                ...
                <div class="modal_footer" slot="footer">
                    <span class="modal_allow" @click="onFire">离职</span>

                    <span class="modal_refuse ml-20" @click="onConfirm">保存</span>
                    <!--<span class="modal_refuse" @click="onCancel" v-if="!noGroup">取消</span>-->
                </div>
            </Modal>
        </no-ssr>
    </div>
...
<script>
  ...
  import {personList} from '~/utils/avatar'
  ...
  export default {
    ...
    methods: {
      ...
      // 员工修改 ok
      async employeeUpdate(obj) {
        let dailysign = await axios.get('/pcapi/employee_manage/employee_info_change', {
          params: Object.assign(obj || {}, this.baseParams)
        })
        const {data} = dailysign
      ...
      addParent() {
        ...
        this.$prompt('请输入分组名称', '新增分组', {
          ...
          inputValidator: function (value) {
            ...
            if (!(that.regexp.title).test(value)) {
              return '分组名称不能超过15个字符'
            }
          }
          // inputPattern: /^[A-Za-z0-9\u4e00-\u9fa5]{1,20}$/,
          // inputErrorMessage: '部门名称不能为空或者不能超过15个字符'
        }).then((value) => {
          let val = value.value
      ...
      // },
      // 详情
      queryDepartment(id, treeNode) {
        const _this = this
        _this.editNode = treeNode
        ...
        _this.employee_info({
          ...
          const elogo = personList.filter(item => item.key === res.elogo)
          ...
          _this.avatar = (elogo[0] && elogo[0]['img']) || _this.avatar
        })
      ...
      // 拖拽之中
      zTreeDragMove: function (event, treeId, treeNodes) {
        // 展开目标node
        // let zTree = null
        // console.log($(event.target).attr('class'))
        // if ($(event.target).attr('class') === 'node_name' && $(event.target).parent().parent().find('ul').length !== 0) {
        // let arr = $(event.target).attr('id').split('_')
        // zTree = $.fn.zTree.getZTreeObj(arr[0])
        // zTree.expandNode(zTree.getNodeByTId(`${arr[0]}_${arr[1]}`))
        // console.log('在有内容的点')
        // } else if ($(event.target).attr('class') === ('panel_head' || 'head_title')) {
        if ($(event.target).attr('class') === ('panel_head' || 'head_title') && $(event.target).parents('.list').find('.panelBody').css('display') === 'none') {
          console.log($(event.target).attr('customdata'))
          this.$emit('onClickPanel', $(event.target).attr('customdata'))
        }
        // console.log(treeId)
        ...
        if (targetNode && targetNode.employee_id) {
          ...
          _this.$confirm('确认移动?', '提示', {
            ...
            let params = {
              ...
              new_department_id: targetNode ? targetNode.department_id : parentTree.diyId,
              old_department_id: treeNodes[0].pId || _this.parentId,
              if_copy: treeNodes[0].isCopy ? '1' : '0'
            }
            _this.employee_changedepartment(params).then(res => {
      ...
      zTreeOnClick: function (event, treeId, treeNode) {
        ...
        if (treeNode.employee_id) {
          ...
          this.queryDepartment(treeNode.employee_id, treeNode)
        }
      },
      // 编辑
        ...
        if (!treeNode.employee_id) {
          ...
          this.$confirm('要将该员工移出部门吗?', '提示', {
            ...
            let params = {
              ...
              employee_id: treeNode.employee_id,
              new_department_id: '',
              old_department_id: treeNode.pId || _this.parentId,
              if_copy: treeNode.isCopy ? '1' : '0'
            }
            _this.employee_changedepartment(params).then(res => {
              zTree.removeNode(treeNode)
      ...
      onFire() {
        const _this = this
        // console.log(_this.editNode)
        // let id = _this.editNode.id
        // let zTree = $.fn.zTree.getZTreeObj(_this.randomId)
        ...
        this.$confirm('确认该人员已离职?', '提示', {
          ...
          _this.employee_delete({
            ...
            // console.log('zTree', zTree)
            zTree.removeChildNodes(_this.editNode)
            zTree.removeNode(_this.editNode)

            _this.$message({type: 'success', message: '人员离职成功!'})
            _this.detail_modal = false
            _this.chenkLength(_this.randomId)
          })
        }).catch(() => {
        })
      ...
      addHoverDom: function (treeId, treeNode) {
        ...
        if (!treeNode.employee_id && _this.edit) {
          let sObj = $('#' + treeNode.tId + '_span')
          if (treeNode.editNameFlag || $('#addBtn_' + treeNode.tId).length > 0) return
          let addStr = "<span class='button add' id='addBtn_" + treeNode.tId + "' title='新增分组' onfocus='this.blur();'></span>"
          sObj.after(addStr)
          let btn = $('#addBtn_' + treeNode.tId)
          if (btn) {
            btn.bind('click', function () {
          ...
          // 为分组员工不显示删除按钮
          if (treeNode.noGroup) {
            sObj.parent().find('.button.remove').hide()
          }
          // 显示复制
          // if (treeNode.editNameFlag || $('#copyBtn_' + treeNode.tId).length > 0) return
          // let addStr = "<span class='button copy' id='copyBtn_" + treeNode.tId + "' title='复制' onfocus='this.blur();'></span>"
          // sObj.after(addStr)
          // let btn = $('#copyBtn_' + treeNode.tId)
          // if (btn) {
          //   btn.bind('click', function (e) {
          //     e.stopPropagation()
          //     let zTree = $.fn.zTree.getZTreeObj(treeId)
          //     let copy = zTree.copyNode(zTree.getNodeByTId(treeNode.pId || null), treeNode, 'next')
          //     copy.isCopy = true
          //     zTree.updateNode(copy)
          //     // console.log(treeNode)
          //     // console.log(copy)
          //   })
          // }
      ...
      onConfirm() {
        ...
        // /employee_manage/employee_info_change
        // user_id	是	string	当前登录的用户id
        // employee_id	是	string	要进行修改操作的员工id
        // change_work_phone	否	string	员工的新工作电话
        // change_email	否	string	员工的新电子邮箱
        // change_work_number	否	string	员工的新工号
        // change_pohoto	否	string	员工的新头像
        this.employeeUpdate({
          user_id: that.$store.state.login.userInfo.uid,
          employee_id: that.editNode.employee_id,
          change_work_phone: that.editForm.tele,
          change_email: that.editForm.mail,
          change_work_number: that.editForm.code,
          change_pohoto: '',
          change_jointime: that.GMTSTR(that.editForm.date).split(' ')[0],
        }).then(res => {
          if (res.code === 0) {
...
<style lang="scss">
    ...
    .ztree li span.button.copy:hover {
